...
function IndexVoting(){
    var r2 = $("#discussion-list").find("tr.discussion-list-item.row").length;
    if(r2 !== r1){
        $("#discussion-list").find("td.voting:not(#cv):not(:has(.pin)):contains('                ')").attr("id","cv").html("<div class='locked'></div>").end()
            .find("ul.upVoted:not(#cv)").attr("id","cv").parent().parent().css("border-left","2px solid #009700").end().end().end()
            .find("ul.downVoted:not(#cv)").attr("id","cv").parent().parent().css("border-left","2px solid #e23636").end().end().end()
            .find("button.up-vote").remove().end()
            .find("button.down-vote").remove();
        r1 = r2;
    }
    var vList = $("#discussion-list").find(".riot-apollo.voting:not(#cv)");
...
function HelpSupport() {
    if($boardIndex > 0){
        if($subBoard == "Help & Support" || $subBoard == "Ask the Community" || $subBoard == "Discuss the Boards"){
                ...
                xmlhttp.onreadystatechange = function() {
                    if (this.readyState == 4 && this.status === 200) {
                        ...
                        if(myObj.discussion.content.hasOwnProperty('pinned') === true && myObj.discussion.hasRioterComments === false){
                            ...
                            if(appID === "okOuNvuT"){
                                //if(appID === "2E4zVIwd"){ //this is testing in clientdisc
