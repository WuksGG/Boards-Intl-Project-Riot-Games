...
(function($,globals){
	...
	$.getScript(`${apolloBaseUrl}/cors/easyXDM.min.js`, ()=> {
		xhr = new easyXDM.Rpc({
			apollo: `${apolloBaseUrl}/cors/index.html`,
			swf: `${apolloBaseUrl}/easyXDM.swf`,
			apolloHelper: `${apolloBaseUrl}/name.html`
		}, {
			remote: {
				request: {} // request is exposed by /cors/
			}
	...
	/** We'll be testing the cross-domain bridge in this section [Begin] **/
	/*
	(function() {
		const remote = 'https://apollo.na.leagueoflegends.com/apollo'
		const appID = 'ZGEFLEUQ'
		const discussionID = 'jLhrLRJy'

		$.getScript(`${remote}/cors/easyXDM.min.js`, ()=> {
			const xhr = new easyXDM.Rpc({
			...
			xhr.request({
				url: `${remote}/applications/${appID}/discussions/${discussionID}?page_size=0&`,
				method: 'GET'
			}, ({ status, data })=> {
				console.log(status)
				console.log(data)
