...
(function($,globals){
	...
	function pullVolunteers(){
		var z = `https://boards.${region}.leagueoflegends.com/api/PlNcL9TL/discussions/MJdEKKbN?page_size=0`;
		//var curr = this;
		...
		if(globals.GLOB.Group1){
			$("a.profile-hover:not(.cka)").each(function(){
				...
				if($this.parent().hasClass("isRioter") === false){
					newFlares(globals.GLOB.Group1,globals.GLOB.Group2,globals.GLOB.Group3,$this);
				} else {
					Tags($this);
				}
			});
	...
	function newFlares(Group1,Group2,Group3,$this){
		var i;
		var apolloID = $this.attr('data-apollo-pvpnet-id');
		for(i=Group1.length-1;i>-1;i-=1){
			if(apolloID === Group1[i]){
				$this.parents('.byline:not(.discussion-footer)').find(".inline-profile:first").append("<span class='tags cwrenchmanoce'><a href='https://boards."+region+".leagueoflegends.com/"+lang+"/c/help-support-oce/nE4lj5EB'>Wrenchman</a></span>").end().end()
		...
		for(i=Group2.length-1;i>-1;i-=1){
			if(apolloID === Group2[i]){
				$this.parents('.byline:not(.discussion-footer)').find(".inline-profile:first").append("<span class='tags cemissary'><a href='https://boards."+region+".leagueoflegends.com/"+lang+"/c/player-behaviour/Edu7E6Bp'>Emissary</a></span>").end().end()
		...
		for(i=Group3.length-1;i>-1;i-=1){
			if(apolloID === Group3[i]){
				$this.parents('.byline:not(.discussion-footer)').find(".inline-profile:first").append("<span class='tags cinstructor'><a href='https://boards."+region+".leagueoflegends.com/"+lang+"/c/announcements-en/91vEPc8g'>Instructor</a></span>").end().end()
	...
	function boardIndexFlares() {
		if($boardIndex > 0 && xhr){
			//console.log(xhr);
			if(["Help & Support (OCE)","Ask the Community","Discuss the Boards","Testing Area"].indexOf($subBoard) > -1){
